...
public class Archers {
    ...
    public static void playGame() {
        for (int i = 0; i < players.length; i++) {
            ...
            players[i].setScores(point);
            System.out.printf("""
                    =============================
                    %s got %d points%n
                    """, players[i].getName(), point);
        }
    }

    public static void displayArchersResult() {
        System.out.printf("""
                ==================================================
        ...
        for (int i = 0; i < players.length; i++) {
            System.out.printf("""
                    ==================================================
                    %-15s""", players[i].getName());
            for (int j = 0; j < 3; j++) {
                System.out.printf("%-11d", players[i].getScores(j));
            }
            System.out.print(players[i].getTotalShot());
            System.out.println();
        }
        displayWinner();
    }
    ...
    public static void archersRound(int[][] players) throws InterruptedException {
        ...
        System.out.printf("%s wins with %d shots points", playerName[2], playersTotal[0]);
